...
<ul>
<li>Give each unique string in <code>original</code> and <code>changed</code> arrays a unique id. There are at most <code>2 * m</code> unique strings in total where <code>m</code> is the length of the arrays. We can put them into a hash map to assign ids.</li>
<li>We can pre-compute the smallest costs between all pairs of unique strings using Floyd Warshall algorithm in <code>O(m ^ 3)</code> time complexity.</li>
<li>Let <code>dp[i]</code> be the smallest cost to change the first <code>i</code> characters (prefix) of <code>source</code> into <code>target</code>, leaving the suffix untouched.
...
If neither of the two conditions is satisfied, <code>dp[i] = infinity</code>.</li>
<li>We can use Trie to check for the second condition in <code>O(1)</code>.</li>
<li>The answer is <code>dp[n]</code> where <code>n</code> is <code>source.length</code>.</li>
</ul>