...
<ul>
<li>Use dynamic programming. dp[i] is max jumps you can do starting from index i. Answer is max(dp[i]).</li>
<li>dp[i] = 1 + max (dp[j]) where j is all indices you can reach from i.</li>
</ul>