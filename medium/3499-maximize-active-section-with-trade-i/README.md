...
<ul>
<li>Split the string into several zero-one segments.</li>
<li>For each one-segment, if it has two neighbors (i.e., it is surrounded by two zero-segments), the total sum of their lengths is one of the candidates for <code>delta</code>.</li>
<li>Find the maximum <code>delta</code> and add it to the total number of ones in the string.</li>
</ul>