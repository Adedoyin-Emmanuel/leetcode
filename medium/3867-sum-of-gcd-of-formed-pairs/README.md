...
<ul>
<li>Maintain a running prefix maximum <code>mx<sub>i</sub></code> while iterating nums to compute <code>prefixGcd[i] = gcd(nums[i], mx<sub>i</sub>)</code>.</li>
<li>Sort <code>prefixGcd</code> in non-decreasing order.</li>
<li>Form pairs by combining smallest unpaired and largest unpaired elements.</li>
<li>Compute gcd for each pair and sum them; ignore middle element if <code>n</code> is odd.</li>
</ul>